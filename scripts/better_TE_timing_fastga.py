...
def calc_matches_mismatches(cigar_string):
    # Regex to find all occurrences of a number followed by '=' for matches, or "X" for mismatches
    # Count matches
    match = r'(\d+)[=]'
    ...
    if 'X' in cigar_string:
        indel = r'(\d+)[X]'
        indels = re.findall(indel, cigar_string, re.IGNORECASE)
        mismatches_bp = sum(int(num) for num in indels)
    else:
        mismatches_bp = 0
...
def align_calc(repeat_types, tmp_dir, threads, coverage):
    # create dictionary for best hits
    best_hits = {}
    threads=f"-T{threads}"

    # loop through repeat familys, aligning with fastga and then calculating best hit
    for repeat_type in repeat_types:
        ...
        paf_path=f"{tmp_dir}/{repeat_type}.fastga.paf"
        # Runs all-vs-all FastGA alignment for repeats in superfamily

        cmd = [
            "FastGA", threads, "-v", "-l60", "-i.7", fasta_path, "-paf"
        ]
        ...
        with open(paf_path) as fh:
                for line in fh:
                    ...
                    # pull out FastGA calculated distance ("Gap-compressed identity")
                    dv = float(tags.get("dv"))
                    
                    best_hits[qname] = [str(dv)]
    return(best_hits)
    ...
    with open(gff_file, "r") as fh, open(out_gff_path, "w") as o:
        for line in fh:
            ...
            if repeat in best_hits:
                dv = float(best_hits[repeat][0])
                out_line=f"{joined_fields};FASTGA_DV={dv:.3f}\n"
            else:
...
def main():
    ...
    best_hits = align_calc(repeat_types, tmp_dir, args.threads, args.coverage)
    end_time = time.time()
    if args.debug:
