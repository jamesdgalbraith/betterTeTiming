...
def align_calc(repeat_types, tmp_dir, threads, coverage, task):
	...
	for repeat_type in repeat_types:
		...
		db_cmd = [
			"makeblastdb", "-dbtype", "nucl", "-in", fasta_path
		]
		...
		with open(blastn_path) as fh:
				for line in fh:
					...
					best_hits[qname] = [str(abs(jc_dist)), pident, qname, tname]
	return(best_hits)
	...
	with open(gff_file, "r") as fh, open(out_gff_path, "w") as o:
		for line in fh:
			...
			if repeat in best_hits:
				jc_dist, de, qname, tname = float(best_hits[repeat][0]), float(best_hits[repeat][1]), best_hits[repeat][2], best_hits[repeat][3]
				out_line=f"{joined_fields};JC_DIST={jc_dist:.3f};PIDENT={de:.3f};QUERY={qname};TARGET={tname}\n"
			else:
...
def main():
	...
	best_hits = align_calc(repeat_types, tmp_dir, args.threads, args.coverage, args.task)
	end_time = time.time()
	if args.debug:
