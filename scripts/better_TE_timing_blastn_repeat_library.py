...
def argsParser():
    ...
    parser.add_argument('-t', '--threads', type=int, default=4,
                        help='number of threads to run blastn with')
    parser.add_argument('-bt', '--task', type=str, default="dc-megablast",
                        help='number of threads to run blastn with')
    ...
    if not args.task in ["dc-megablast", "megablast", "blastn"]:
        sys.exit("BLASTN task not valid")
...
def align_calc(repeat_types, tmp_dir, threads, coverage, task, library):
    # create dictionary for best hits
    best_hits = {}
    db_cmd = [
        "makeblastdb", "-dbtype", "nucl", "-in", library
    ]
    subprocess.run(db_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    for repeat_type in repeat_types:
        ...
        # makeblastdb for fasta
        # Runs blastn alignment for repeats in superfamily
        fasta_path=f"{tmp_dir}/{repeat_type}.fa"
        blastn_path=f"{tmp_dir}/{repeat_type}.blastn"
        outfmt = f"6 qseqid qlen qstart qend sseqid nident mismatch pident positive"
        blast_cmd = [
        ...
        blast_run = subprocess.run(blast_cmd, capture_output=True, text=True)
        
        # Read through paf and calculate jc distance
        with open(blastn_path) as fh:
                for line in fh:
                    cols = line.rstrip("\n").split("\t")
                    ...
                    best_hits[qname] = [str(abs(jc_dist)), pident, tname]
    return(best_hits)
    ...
    with open(gff_file, "r") as fh, open(out_gff_path, "w") as o:
        for line in fh:
            ...
            if repeat in best_hits:
                jc_dist, pident, tname = float(best_hits[repeat][0]), float(best_hits[repeat][1]), best_hits[repeat][2].split("#")[0]
                out_line=f"{joined_fields};JC_DIST={jc_dist:.3f};PIDENT={pident:.3f};TARGET={tname}\n"
            else:
...
def main():
    ...
    best_hits = align_calc(repeat_types, tmp_dir, args.threads, args.coverage, args.task, args.library)
    end_time = time.time()
    if args.debug:
